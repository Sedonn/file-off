...
const loadLang = (key: string): Lang =>
    JSON.parse(fs.readFileSync(path.join(__dirname, `${key}.json`), 'utf-8'));
...
export const getValidLang = (lang: string): keyof Langs =>
    (LANGS.includes(lang) ? lang : DEFAULT_LANG) as keyof Langs;
...
export const langsInit = (): Langs =>
    Object.assign({}, ...LANGS.map((lang) => ({ [lang]: loadLang(lang) })));
